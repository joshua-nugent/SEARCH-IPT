...
summarize_continuous_ms <- function(results, version = "complex_covars"){
  output <- results %>% filter(link_function == "identity",
    ...
    summarize(n = n(),
              ...
              #glmm_mean_se = mean(ltmle_se_break_CV, na.rm = T),
              gee_mean_se = mean(ltmle_se_ignore_clustering, na.rm = T),

              tmle_pe_sd = sd(ltmle_pe, na.rm = T),
              #glmm_pe_sd = sd(ltmle_pe_break_CV, na.rm = T),
              gee_pe_sd = sd(ltmle_pe_ignore_clustering, na.rm = T),
              ...
              #glmm_mse = mean((ltmle_pe_break_CV - t_eff)^2, na.rm = T),
              gee_mse = mean((ltmle_pe_ignore_clustering - t_eff)^2, na.rm = T),

              tmle_semse = sd((ltmle_pe - t_eff)^2, na.rm = T) / sqrt(n),
              #glmm_semse = sd((ltmle_pe_break_CV - t_eff)^2, na.rm = T) / sqrt(n),
              gee_semse = sd((ltmle_pe_ignore_clustering - t_eff)^2, na.rm = T) / sqrt(n),
              ...
              #glmm_TIE = mean(ltmle_lo_break_CV > 0 | ltmle_hi_break_CV < 0),
              gee_TIE = mean(ltmle_lo_ignore_clustering > 0 | ltmle_hi_ignore_clustering < 0),

              tmle_power = mean(ltmle_lo > 0 | ltmle_hi < 0, na.rm = T),
              #glmm_power = mean(ltmle_lo_break_CV > 0 | ltmle_hi_break_CV < 0, na.rm = T),
              gee_power = mean(ltmle_lo_ignore_clustering > 0 | ltmle_hi_ignore_clustering < 0, na.rm = T),

              tmle_cov = mean(ltmle_lo < t_eff & ltmle_hi > t_eff, na.rm = T),
              #glmm_cov = mean(ltmle_lo_break_CV < t_eff & ltmle_hi_break_CV > t_eff, na.rm = T),
              gee_cov = mean(ltmle_lo_ignore_clustering < t_eff & ltmle_hi_ignore_clustering > t_eff, na.rm = T),

              tmle_bias = mean(ltmle_pe - t_eff, na.rm = T),
              #glmm_bias = mean(ltmle_pe_break_CV - t_eff, na.rm = T),
              gee_bias = mean(ltmle_pe_ignore_clustering - t_eff, na.rm = T),
              ...
              #glmm_cise_hi = glmm_mean_se + 1.96*sqrt(var(ltmle_se_break_CV) / (4*n*glmm_mean_se^2)),
              gee_cise_hi = gee_mean_se + 1.96*sqrt(var(ltmle_se_ignore_clustering) / (4*n*gee_mean_se^2)),

              tmle_cise_lo = tmle_mean_se - 1.96*sqrt(var(ltmle_se) / (4*n*tmle_mean_se^2)),
              #glmm_cise_lo = glmm_mean_se - 1.96*sqrt(var(ltmle_se_break_CV) / (4*n*glmm_mean_se^2)),
              gee_cise_lo = gee_mean_se - 1.96*sqrt(var(ltmle_se_ignore_clustering) / (4*n*gee_mean_se^2)),
...
get_facet <- function(dat, style, type = "bias"){
  ...
  if(style == "balanced"){
    ...
    versions <- paste0("control clusters = ", dat$control_arm_clusters,
                       ...
                       "\ntreatment clusters = ", dat$treatment_arm_clusters,
...
# bias
##########################################
generate_bias_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
  ...
  plotdat <- dat %>% #filter(t_eff != 0) %>% 
    ...
    mutate(`Control arm modeled as` = ifelse(str_detect(method, "_cl"),"clustered", "unclustered"),
           ...
           method = ifelse(method == "gee_", "TMLE without IC aggregation", ifelse(method == "glmm", "CV ignores clustering", "TMLE as proposed")))
...
# MSE
##########################################
generate_MSE_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
...
# TIE
##########################################
generate_TIE_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
...
# coverage
##########################################
generate_coverage_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
...
# power
##########################################
generate_power_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
  style <- dat[["style"]][[1]]
...
##########################################
generate_se_plot <- function(dat, dodge_width = .4, exclude_clustered = T){
...
##########################################
generate_pe_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
...
##########################################
generate_var_ratio_plot_ms <- function(dat, dodge_width = .4, exclude_clustered = T){
  style <- dat[["style"]][[1]]
...
##########################################
generate_legend_plot_ms <- function(dat, exclude_clustered = T){
