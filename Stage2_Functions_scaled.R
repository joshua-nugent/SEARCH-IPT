...
#-----------------------------------------------------#-----------------------------------------------------
get.IC.variance <- function(goal, Vdata, R1, R0, scale_value = 1, scale_value_min = 0){
...
get.inference <- function(goal, R1, R0, var.IC, df, sig.level=0.05, one_sided = T){
  ...
  if(one_sided){
    ...
    pval <- pt(abs(tstat), df = df, lower.tail=F) 
  } else {
    #cutoff <- qt(sig.level/2, df = df, lower.tail=F)
